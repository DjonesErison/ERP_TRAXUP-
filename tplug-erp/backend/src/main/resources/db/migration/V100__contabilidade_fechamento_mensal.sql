INSERT INTO permissoes (id, chave, descricao) VALUES
    (
        ...
        'Consultar resumo mensal de fechamento contabil'
    )
...
JOIN permissoes perm
  ON perm.chave = 'CONTABILIDADE_FECHAMENTO_LER'
WHERE UPPER(p.nome) IN ('ADMIN', 'CONTABILIDADE')
ON CONFLICT DO NOTHING;

CREATE INDEX idx_inventario_sessoes_fechamento_contabil
    ON inventario_sessoes (tenant_id, concluido_em DESC, filial_id)
