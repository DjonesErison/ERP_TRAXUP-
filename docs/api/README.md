...
A autorização dos endpoints operacionais usa authorities derivadas do claim `permissions` do JWT. A API administrativa de RBAC nunca aceita tenant por header ou payload: o tenant é derivado exclusivamente do JWT e os vínculos entre tenants diferentes são rejeitados. OpenAPI/Swagger ainda está planejado e não deve ser considerado implantado.
