  ...
  template:`<section class="page">
    ...
    <div class="two">
      <div class="card panel"><div class="panel-title"><h2>Visão global automática</h2><a routerLink="/funcionalidades">Ver backlog</a></div>
        @for(k of kinds;track k.name){<div class="bar-row"><span>{{k.name}} <small>({{k.items}} itens rastreados)</small></span><div class="track"><i [style.width.%]="k.value"></i></div><b>{{k.value}}%</b></div>}
        <p><small>Metodologia: Concluída 100% · Em implementação 75% · Especificada 50% · Planejada 10%. Itens decompostos podem compartilhar o peso de escopo do item original, evitando inflar o progresso apenas por aumentar a quantidade de linhas do backlog.</small></p>
      </div>
    ...
    <div class="card panel" style="margin-top:18px"><div class="panel-title"><h2>Progresso automático por frente</h2><span class="badge">{{modules.length}} frentes</span></div>@for(m of modules;track m.name){<div class="bar-row"><span>{{m.name}} <small>({{m.items}} itens rastreados)</small></span><div class="track"><i [style.width.%]="m.value"></i></div><b>{{m.value}}%</b></div>}</div>
...
export class DashboardComponent implements OnInit {
  ...
  activities=['CI, Docker e deploy automático da Central validados','Documento Mestre e jornadas consolidados','Catálogo visual oficial publicado na Central','API de Perfil Fiscal por Filial implementada','Contabilidade decomposta em entregas rastreáveis com evidências reais'];
