...
@Component({selector:'app-reenviar-ativacao', standalone:true, imports:[CommonModule,FormsModule],
 ...
 <label>Empresa (4 dígitos)<input name="empresaAtivacao" [(ngModel)]="codigoEmpresa" required pattern="[0-9]{4}" maxlength="4" inputmode="numeric" autocomplete="organization"></label>
...
export class ReenviarAtivacaoComponent {
 @Input() codigoEmpresa=''; @Input() email=''; loading=false; mensagem=''; error='';
 constructor(private readonly trial:TrialService){}
 enviar():void {
   if(this.loading)return;
   this.loading=true;this.mensagem='';this.error='';
   this.trial.reenviar(this.codigoEmpresa.trim(),this.email.trim()).subscribe({
