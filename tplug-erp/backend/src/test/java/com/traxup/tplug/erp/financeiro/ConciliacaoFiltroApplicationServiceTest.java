...
class ConciliacaoFiltroApplicationServiceTest {
    ...
    void deveNormalizarFiltrosEPreservarTenantEConta() {
        ...
        when(repository.filtrar(tenantId, contaId, "OFX", "TAXA", "PENDENTE", inicio, fim)).thenReturn(List.of());

        novoService().listar(tenantId, contaId, " ofx ", "taxa", "pendente", inicio, fim);

        verify(repository).filtrar(tenantId, contaId, "OFX", "TAXA", "PENDENTE", inicio, fim);
    }
